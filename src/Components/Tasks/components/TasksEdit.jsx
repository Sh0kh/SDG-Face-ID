...
export default function TasksEdit({ data, refresh }) {
    ...
    const handleEdit = async () => {
        ...
        try {
            ...
            await $api.put(`/permission/update`, EditData, {
                headers: {
                    'ngrok-skip-browser-warning': 'true'
                }
            });
            Alert("Vazifa muvaffaqiyatli yangilandi", "success");
            handleOpen();
            refresh();
        } catch (error) {
            console.error(error);
            Alert("Vazifani yangilashda xatolik yuz berdi", "error");
        } finally {
            setLoading(false);
        }
    };
